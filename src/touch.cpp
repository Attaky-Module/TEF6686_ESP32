  ...
  if (scandxmode) {
    ...
    if (!BWtune && !menu && !advancedRDS && !seek && !afscreen) {         // Normal radio mode
      if (x > 0 && x < 320 && y > 180 && y < 240 && band < BAND_GAP) {    // -----------------
...
#ifdef ATK_COMBO_V1
        // Board profile: frequency digits area. MAN = direct-entry keypad
        // (upstream b02d3f1 semantics). AUTO = seek, direction by tapped
        // half. MEM = cycle to the next preset slot. DX scan stays in the
        // DX mode menu.
        if (tunemode == TUNE_MEM) {
          KeyUp();
        } else if (tunemode == TUNE_AUTO) {
          if (x < 150) KeyDown(); else KeyUp();
        } else {
          freqkeypadtune = true;
          BuildFreqKeypad();
        }
        ...
        return;
#ifdef ATK_COMBO_V1
      // The panel's physical left edge maps to screenX ~28-39 (HW-measured
      // taps), so the upstream x < 30 bound is unreachable here. The lower
      // y bound keeps clear of the MENU hamburger zone above it.
      } else if (x > 0 && x < 45 && y > 32 && y < 95) {
#else
...
#ifdef ATK_COMBO_V1
        // Board profile: in MEM mode this pill doubles as the preset-store
        // key (first tap arms, second tap saves — ButtonPress semantics);
        // MAN/AUTO keep the iMS/EQ toggle.
        if (tunemode == TUNE_MEM) ButtonPress(); else toggleiMSEQ();
#else
